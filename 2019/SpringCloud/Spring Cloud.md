...
![title](../../.local/static/2020/0/3/1578449864151.1578449864160.png)
现在出现了网络分区后，此时有一个请求过来了，想要注册一个账户
![title](../../.local/static/2020/0/3/1578449903378.1578449903385.png)

此时我们**节点一和节点三是不可通信**的，这就有了抉择：

- 如果**允许当前用户注册**一个账户，此时注册的记录数据**只会**在节点一和节点二或者节点二和节点三同步，因为节点一和节点三的记录不能**同步**的。
  - 这种情况其实就是选择了可用性(availability)，抛弃了数据一致性(consistency)

- 如果**不允许当前用户注册**一个账户(就是要**等到**节点一和节点三恢复通信)。节点一和节点三一旦恢复通信，我们就可以保证**节点拥有的数据是最新版本。**
...
4.1为什么需要SpringCloud？
前面也讲了，从分布式/微服务的角度而言：就是把我们一**大的项目，分解成多个小的模块**。这些小的模块组合起来，完成功能。
...
# 五、引出Eureka
那会出现什么问题呢？？首当其冲的就是子系统之间的**通讯**问题。子系统与子系统之间不是在同一个环境下，那就需要远程调用。**远程调用**可能就会想到httpClient，WebService等等这些技术来实现。
...
![title](../../.local/static/2020/0/3/1578450893412.1578450893419.png)
...
![title](../../.local/static/2020/0/3/1578451048578.1578451048578.png)
...
最后，我们就有了这张图：
![title](../../.local/static/2020/0/3/1578451391108.1578451391114.png)
...
![title](../../.local/static/2020/0/3/1578451516918.1578451516925.png)
...
所以，我们的图可以画成这样：
![title](../../.local/static/2020/0/3/1578451582534.1578451582544.png)
...
比如我们的Demo中，每个服务都写上相同的配置文件。万一我们有一天，配置文件中的密码需要更换了，那就得三个**都要重新更改**。
...
![title](../../.local/static/2020/0/3/1578489704258.1578489704264.png)
